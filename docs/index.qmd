...
```{r, include=FALSE}
# ❌❌❌❌❌❌❌❌❌ (env_var
# Join hbk detection and gld + arc background mission summaries by Oceanographic_Variable col
join_summary <- hbk_summary %>%
  left_join(gld_summary %>% 
              select(
...
```{r, include=FALSE}
# ❌❌❌❌❌❌❌❌❌ (env_vars_gld) + fix outlier ranges for new vars
...
gld_long$boxorder <- factor("Background", levels = c("Detection", "Background"))
...
# DETECTION box (colors by variable, keep outliers)
  ...
  geom_boxplot(data = gld_long, aes(x = boxorder, y = Value),
               fill = "grey80",
               color = "grey40",
               width = 0.9,
               alpha = 0.45,
